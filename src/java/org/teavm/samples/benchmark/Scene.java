...
public class Scene {
    ...
    private void initReel() {
        ...
        int parts = 30;
        double partsMinOne = parts - 1.0;
        for (int i = 0; i < parts; ++i) {
            PolygonShape shape = new PolygonShape();
            double angle1 = (i / partsMinOne) * 2.0 * Math.PI;
            double x1 = 2.7 * Math.cos(angle1);
            double y1 = 2.7 * Math.sin(angle1);
            double angle2 = ((i + 1) / partsMinOne) * 2.0 * Math.PI;
            ...
            shape.set(new Vec2[] {
                new Vec2((float)x1, (float)y1),
                new Vec2((float)x2, (float)y2),
                new Vec2((float)(x2 - x), (float)(y2 - y)),
                new Vec2((float)(x1 - x), (float)(y1 - y))}, 4);
            fixture.shape = shape;
